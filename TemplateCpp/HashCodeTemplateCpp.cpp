...
int main()
{
    ...
    for(auto& p : directory_iterator("input")) 
    {
        ...
        Dataset dataset = parser.GetDataFromStream(inputFilePath, filename);
        Solution solution = solver.GetTheSolution(dataset);
        printer.PrintSolution(solution, filename);

        // Below these lines are optional, can be removed if not necessary
        //long long score = scoring.CalculateScore(dataset, solution);
    }
    return 0;
}
